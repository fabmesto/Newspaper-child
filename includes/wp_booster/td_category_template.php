...
abstract class td_category_template {
     ...
     * @throws ErrorException
     */
    protected function fab_get_sibling_categories($params_array = '') {
        $buffy = '';
         ...
         */
        if (!empty($categories_objects)) {
            $buffy = '<div class="fab-td-category-siblings">'; // FAB
                $buffy .= '<div class="td-block-row">'; // FAB
                    foreach ($categories_objects as $category_object) {
                        ...
                        if (!empty($category_object->name) and td_util::get_category_option($category_object->cat_ID,'tdc_hide_on_post') != 'hide') {
                            ...
                            $buffy .= '<div class="td-block-span4 ' . $class . '">'; // FAB
                            $buffy .= '<div class="td_module_3 td_module_wrap td-animation-stack">';
                            // FAB START
                            if (function_exists('z_taxonomy_image_url')):
                              $buffy .= '<div class="td-module-image">';
                              $buffy .= '<div class="td-module-thumb">';
                              $buffy .= '
          										<a href="'.get_category_link($category_object->term_id).'" title="'.$category_object->name.'">
          											<img width="324" height="235" src="'.z_taxonomy_image_url($category_object->term_id).'" alt="" />
          										</a>';
                              $buffy .= '</div>';
                              $buffy .= '</div>';
                            endif;
                            ...
                            $buffy .= '<a ' . $td_css_inline->get_inline_css() . '  href="' . get_category_link($category_object->cat_ID) . '">' . $category_object->name . '</a>';
                            $buffy .= '</h3>';
